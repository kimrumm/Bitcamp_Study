...
-- rollback 잘못 했으면 이전으로 돌리기 (반영하지않음, 커밋이랑 반대)
...
-- update 테이블명 set 컬럼이름=새로운 데이터, 컬럼이름=새로운데이터 where 행을 선택하는 조건
...
-- 업데이트나 딜리트는 조건에 따라서 하나를 받거나 여러개를 받을 수 있다.
update dept03
set loc='SEOUL'
;
--where 모든 부서라 조건 필요 x 
