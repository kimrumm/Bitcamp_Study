...
select substr (hiredate,1,2) as "입사년도", substr (hiredate,4,2) as "입사달"
from emp
;
...
select trunc(sysdate-to_date('2021/01/01','YYYY/MM/DD')) as "올해 몇 칠이 지났는가"
from dual;
...
--24. 각 담당 업무 유형별로 -> job 그룹핑)
-- 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. -> 그룹함수
-- 평균에 대해서는 정수로 반올림 하시오. -> 
select job, max(sal) as "급여최고액",
            min(sal) as "급여최소액",
            sum(sal) as "총액",
            round(avg(sal)) as "평균급여"
from emp
...
select max(sal)-min(sal) as "최고 최저 급여 차액"
from emp
;
...
select deptno, count(*)"사원수", round(avg(sal),2)"모든사원평균급여"
from emp
group by deptno
;
