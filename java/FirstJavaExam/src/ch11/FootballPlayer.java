...
public class FootballPlayer implements Comparable<FootballPlayer> {
	...
	public int compareTo(FootballPlayer o) {
		...
		int result = this.team.compareTo(o.getTeam())*-1;	//음수 / 양수 / 0이 나올 수도 있다.
		if(result == 0) {
	...
	public static void main(String[] args) {
		...
		//List<FootballPlayer> players = new ArrayList<>(); // 커맨드 쉬프트 o -> 임포트

		// 데이터저장
		players.add(new FootballPlayer("son1", 7, "Tot", 27));
		players.add(new FootballPlayer("Park1", 7, "Man", 37));
		players.add(new FootballPlayer("son1", 9, "Tot", 17)); // 2번 문제와 비슷함
		players.add(new FootballPlayer("son2", 10, "Man", 37));
		players.add(new FootballPlayer("Park2", 20, "Man", 31));
		players.add(new FootballPlayer("Park2", 21, "Man", 31)); // 중복저장 되는지 여부 확인
		...
		while (itr.hasNext()) {
			System.out.println(itr.next()); // FootballPlayer 를 반환한다.
		}
		...
		players2.put(10, new FootballPlayer("Kane", 7, "Tot", 26));
		players2.put(10, new FootballPlayer("Kane2", 20, "Tot", 16));
